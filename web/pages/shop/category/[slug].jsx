...
export const ShopCategory = ({ data, slug }) => {
  ...
  const currentTag = tags.find(
    (tag) => tag.value.current.toLowerCase() === slug
  );

  const filteredProducts = products.filter((prod) =>
    prod?.tags?.length ? prod.tags[0].value.toLowerCase() === slug : null
  );
  ...
  return (
    <>
      <NextSeo
        ...
        canonical={`${process.env.NEXT_PUBLIC_BASE_URL}/shop/category/${currentTag.value?.current}`}
      />
